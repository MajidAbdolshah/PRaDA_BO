...
def createPoints(deepness,breadth):
    ...
    for i in range(1,deepness+1):
        ...
        temp_x = np.linspace(0, i, num=breadth)
...
if __name__ == '__main__':
    ...
    xData = np.loadtxt('xData_.txt', dtype=float)
    ...
    while ctn < COUNTER:
        ...
        if (SlidingY.shape[0] > 16):
            indices_ = [random.randint(0, SlidingY.shape[0]-1) for p in range(0, 16)]
        ...
        plt.plot(yPareto[:,0],yPareto[:,1],"or",markersize = 8)
