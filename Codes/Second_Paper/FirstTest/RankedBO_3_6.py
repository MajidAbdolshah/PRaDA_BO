...
def createPoints(deepness,breadth):
    ...
    for i in range(1,deepness+1):
        ...
        temp_x = np.linspace(0, i/6, num=breadth)
...
if __name__ == '__main__':
    ...
    while ctn < COUNTER:
        ...
        if (SlidingY.shape[0] > 32):
            indices_ = [random.randint(0, SlidingY.shape[0]-1) for p in range(0, 32)]
    ...
    plt.legend()
    plt.title('r2>(6)*r1')
    plt.savefig('6times.pdf', format='pdf', dpi=1000)
    plt.show()
